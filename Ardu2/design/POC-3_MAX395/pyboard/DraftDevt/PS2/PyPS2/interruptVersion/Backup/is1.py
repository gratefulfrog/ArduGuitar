...
The way this works is that the host sends a command and the device replies.  I have NOT IMPLEMENTED STREAMING MODE! 
...
sometimes causes the device to freeze, sigh...
...
8. run(out,bitsInn,bitsOut)
   ...
   a call to init seems to help prevent freezing, but I don't know why!!!
...
9. c(tup)
   sets up the call to run with tup[1],tup[0]
10. check(v)
